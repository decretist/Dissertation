...
prose for which computational stylometry works is about 2,500 words.[10]
...
recension ends with D.99 c.1.[13]
...
different from the original text, but is defined by the deformances
through which it is generated from the original. The classic example of
deformance in a literary context is reversing the order of lines in a
poem. The paratext can be analyzed for otherwise unavailable insights
...
In 1946, Father Roberto Busa, SJ (d.2011), began work on what ultimately
became the *Index Thomisticus*, a concordance of the works of St Thomas
Aquinas. In 1949, Father Busa secured crucial support from Thomas J.
Watson of IBM, allowing concordance generation to be carried out by
...
origin stories for digital humanities as a discipline.[18] The success
...
major Busa-style concordance. For this reason, the MGH e-text of the
Friedberg edition was encoded in the obsolete Oxford Concordance Program
(OCP) format.
...
*dicta* was implemented in the form of a 201-line Python program.[22]
...
content, and end tag for the top-level section heading element comes
...
C.19 d.init.[24] The first-recension version of the text of C.19 d.init.
...
contulit*). Gratian0, the sample containing the case statements, is
therefore made up of 99.6% first-recension text. Similarly, the number
of words added to the *dicta* in *de Penitentia* between the first and
second recensions is relatively small, 556 words out of a total of
10,081 words; dePen, the sample containing the *dicta* from *de
Penitentia*, is therefore made up of 94.5% first-recension text.
...
<img src="https://render.githubusercontent.com/render/math?math=s=\\sqrt{\\frac{1}{N - 1}\\sum\_{i=1}^N(x\_i-\\bar{x})^2}">
...
extended discussions of the phenomenon.[27] **The rank-frequency
...
word.[28] In other words, the theoretical Zipf distribution predicts
...
words.[29] Zipf had a vivid awareness, ahead of its time for the
...
discrete rather than continuous quantities.[30] **Zipf also noted that
value of the exponent of b is likely to differ from 2 depending on the
size of the corpus[31]**
...
reasonably well.[32] The rank-frequency relationship does not on first
...
words from Gratian's *dicta*.[33] (See Figure Zd below.) Transposing
...
(*Δ*<sub>*B*</sub>).[35] Expositions of Burrows's Delta sometime fail to
...
proposed since,[36] Burrows's Delta is widely accepted in the scholarly
...
statements.[37] As noted in Chapter 0 above, Gratian can be said with a
...
(Gratian2) have the same author. It is less likely still that the
...
Penitentia*).[38] For each of the fourteen subcorpora, we will
...
[13] Kenneth Pennington, “The Biography of Gratian, the Father of Canon
Law,” *Villanova Law Review* 59 (2014): 685.

[14] **Anders, my understanding is that you took a similar approach when
...
[22] Python is a widely-used general-purpose programming language.
...
operations on textual data, notably regular expressions, which allow
programmers to specify arbitrarily complicated patterns of characters
according to a rule, and then to search for and replace units of text
matching the pattern. (The use of the word *regular* in the term
*regular expression* is analogous to its use in the term *canons
regular*. In both cases a rule is being followed.)

[23] Hockey, “The History of Humanities Computing.”

[24] Winroth, *The Making of Gratian’s Decretum*, 216.

[25] Much of the analysis from this point forward will take advantage of
...
[26] The actual generation of Figure 0a was deferred until after the
...
[28] **In Digital Humanities courses, 1/N is typically presented *as*
...
[29] "Hence the *a**b*<sup>2</sup> = *k* relationship is valid only for
